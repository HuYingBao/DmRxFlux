...
public class ActionCreator extends BaseRxActionCreator implements Actions {
    ...
    public void connectServer() {
        //连接服务器，先切换内网服务器
        ServerUtils.setServerState(true);
        RxAction action = newRxAction(CONNECT_SERVER);
        // TODO 若是外网连接成功，会调用两次连接服务器接口
        postHttpActionNoCheck(action, mHttpApi.connectServer()
                .retryWhen(throwableObservable -> throwableObservable.flatMap(throwable -> {
                    //内网服务器连接不成功，连接外网服务器
                    ServerUtils.setServerState(false);
    ...
    public void login(Context context, String name, String pwd, String channelId) {
        ...
        //先使用内网登录
        ServerUtils.setServerState(true);
        postLoadingHttpActionNoCheck(context, rxAction,
                mHttpApi.login(rxAction.getData())
                        .flatMap(verifyResponse())
                        .retryWhen(throwableObservable -> throwableObservable.flatMap(throwable -> {
                            //内网登录不成功，切换外网
                            ServerUtils.setServerState(false);
                            //若是外网连接成功，再重新登录；若是外网连接不成功，抛出异常
                            return mHttpApi.connectServer();
                        })));
    }
    ...
    @Override
    public void getAllMsg(int page, int rows) {
        RxAction action = newRxAction(GET_ALL_MSG,
                ActionsKeys.ROWS, rows,
                ActionsKeys.PAGE, page);
        postHttpAction(action, mHttpApi.getAllMsg(action.getData()));
    }
    ...
    @Override
    public void getDeviceList(String arrayType, String arrayStatus, int page) {
    ...
    public void repairPause(Context context, int repairId, String faultCause, String scheme, String remark, String pauseExplain, List<DevicePart> devicePartList) {
        ArrayMap<Integer, String> spareparts = new ArrayMap<>();
