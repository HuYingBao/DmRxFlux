...
public abstract class BaseRxFluxActivity extends RxAppCompatActivity implements RxViewDispatch {
    ...
    public void onRxError(@NonNull RxError error) {
        switch (error.getAction().getType()) {
            case Actions.CONNECT_SERVER:
                showShortToast("连接服务器失败，请检查网络是否连接成功！");
                break;
            default:
    ...
    private void handleThrowable(@NonNull RxError error) {
        ...
        if (throwable instanceof RxHttpException) {
            ...
            if (TextUtils.equals(((RxHttpException) throwable).code(), Constants.ERROR_SESSION_TIMEOUT)) {
                ConfirmFragment.newInstance(Actions.CLICKED_LOGOUT, "登录过期，请重新登录！", false)
                        .show(getSupportFragmentManager(), ConfirmFragment.class.getSimpleName());
            } else {
